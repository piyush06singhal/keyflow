  ...
  try {
    ...
    if (navigator.onLine) {
      try {
        ...
        // Update heatmap
        try {
          const heatmapResult = await updateHeatmapForSession(userId, sessionResult.duration);
    ...
    if (navigator.onLine) {
      try {
        ...
        if (syncResult.successful > 0) {
          result.warnings.push(
            `Synced ${syncResult.successful} pending session(s)`
          );
        }
      } catch (error) {
    ...
    try {
      ...
      if (syncStatus.isPending && syncStatus.pendingCount > 0) {
        result.warnings.push(
          `${syncStatus.pendingCount} session(s) pending sync`
        );
      }
    } catch (error) {
      // Silent fail
